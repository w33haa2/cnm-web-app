<template>
  <div class="app-container">
    <div class="title-bar shadow">
      <el-row :gutter="8">
        ...
        <el-col :md="{ span: 4 }">
          ...
          >
            <el-option
              v-for="(option, index) in filter.options.vto_list"
              :key="index"
              :value="option.value"
              :label="option.label"
            />
    ...
    >
      ...
      <el-progress
        :percentage="form.report.progress"
        :text-inside="true"
        :stroke-width="18"
      ></el-progress>
      <div style="padding-bottom:15px;  ">
        ...
        >
          <el-tab-pane
            :label="'All: ' + form.report.data.all.length"
            name="all"
          >
          ...
          <el-tab-pane
            :label="
              'Errors: ' +
                form.report.data.all.filter(i => i.status_code != 200).length
            "
            name="errors"
          >
            <el-table
              :data="form.report.data.all.filter(i => i.status_code != 200)"
              height="350px"
            >
              <el-table-column label="Name" width="350">
                <template scope="scope">{{ scope.row.full_name }}</template>
        ...
        <el-button size="mini" @click="closeCreateVtoReport" style="float:right"
          >Close</el-button
        >
      </div>
    </el-dialog>
...
export default {
  ...
  watch: {
    ...
    fetchVtoListState({ initial, success, fail }) {
      ...
      if (success) {
        this.filter.options.vto_list = this.fetchVtoListData.agent_schedules
          .map(i => ({
            value: moment(i.date).format("YYYY-MM-DD HH:mm:ss"),
            label: moment(i.date).format("YYYY-MM-DD HH:mm:ss")
          }))
    ...
    loopCreateVto() {
      ...
      field.agents.forEach(
        ((v, i) => {
          ...
          formData.append(
            "timestamp",
            moment(field.vto_at).format("YYYY-MM-DD HH:mm:ss")
          );
          axios
            ...
            .then(res => {
              ...
              tmp_data.full_name =
                res.data.meta.agent_schedule.user_info.full_name;
              tmp_data.status_code = res.status;
              tmp_data.title = res.data.title;
              tmp_arr.push(tmp_data);
              this.form.report.data.all = tmp_arr;
              this.form.report.progress =
                (this.form.report.loop_index / this.form.report.arr_length) *
                100;
            })
            .catch(err => {
              loop_count += 1;
