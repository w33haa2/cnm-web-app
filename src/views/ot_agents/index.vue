  ...
  methods: {
    ...
    exportAgentOt(){
      ...
      axios.get(url,this.request.options).then(res=>{
        ...
        result = result.map(i=>([i.user_info.full_name,moment(i.time_in.date).format("YYYY-MM-DD hh:mm a"),moment(i.time_out.date).format("YYYY-MM-DD hh:mm a"),i.conformance+"%"]));
        result.unshift([]);
